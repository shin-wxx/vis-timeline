...
import DataZoom from "./component/DataZoom.js";
...
export default class Timeline extends Core {
  ...
  constructor(container, items, groups, options) {
    ...
    this.body = {
      ...
      util: {
        ...
        // Expose the TimeAxis's fully-configured TimeStep (moment/options/format/
        // hiddenDates/scale/step) so other components (e.g. DataZoom) can reuse
        // the exact same tick-generation algorithm and stay aligned with the
        // main axis. Returns undefined before the first TimeAxis redraw.
        getTimeStep() {
          return me.timeAxis.step;
        },
    ...
    // initialize DataZoom component (registered into this.components)
    this._initDataZoom();
  }

  /**
   * Initialize the DataZoom component and wire up filter refresh listeners.
   * @private
   */
  _initDataZoom() {
    const dzOpts = this.options.dataZoom || {};
    this.dataZoom = new DataZoom(this.body, dzOpts);
    this.components.push(this.dataZoom);

    // 过滤选区变化时刷新 DataView（dataZoom 拖动/滚轮/点击只改过滤比例）
    this.body.emitter.on("datazoomfilterchange", () => {
      this._refreshDataZoomView();
    });

    // 时间轴 pan/zoom 后，过滤区间绝对范围按比例变化，刷新 DataView
    this.on("rangechanged", () => {
      this._refreshDataZoomView();
    });

    // dataZoom 已就绪，刷新 DataView 以应用过滤（setItems 早于 _initDataZoom）
    this._refreshDataZoomView();
  }

  /**
   * Refresh the DataZoom DataView with a cached filter range.
   * Caching avoids calling getFilterRange() per item during refresh.
   * @private
   */
  _refreshDataZoomView() {
    if (!this._dataZoomView) return;
    this._cachedFilterRange = this.dataZoom
      ? this.dataZoom.getFilterRange()
      : null;
    this._dataZoomView.refresh();
  }

  /**
   * Whether DataZoom data filtering is active.
   * @return {boolean}
   * @private
   */
  _isDataZoomFilterEnabled() {
    return !!(
      this.options.dataZoom &&
      this.options.dataZoom.enabled &&
      this.options.dataZoom.filterData
    );
  }

  /**
   * Wrap the given coerced dataset in a DataView filtered to the dataZoom
   * filter range and feed it to the ItemSet.
   * @param {Object|null} dataSet  The coerced dataset wrapper from setItems
   * @private
   */
  _setupDataZoomFilter(dataSet) {
    if (dataSet == null) {
      this._dataZoomView = null;
      this.itemSet.setItems(null);
      return;
    }
    this._dataZoomView = new DataView(dataSet.rawDS, {
      filter: (item) => this._dataZoomItemFilter(item),
    });
    this.itemSet.setItems(this._dataZoomView);
  }

  /**
   * DataView filter predicate: keep items that overlap the dataZoom filter
   * range (a sub-range of the visible window, controlled by the dataZoom
   * selection ratios). Uses cached filter range for performance.
   * @param {Object} item
   * @return {boolean}
   * @private
   */
  _dataZoomItemFilter(item) {
    // dataZoom 可能在 setItems 之后才初始化（构造函数顺序），此时暂不过滤
    const fr = this._cachedFilterRange;
    if (!fr) return true;
    const itemStart = util.convert(item.start, "Date").valueOf();
    const itemEnd = util
      .convert(item.end != null ? item.end : item.start, "Date")
      .valueOf();
    return itemEnd >= fr.start && itemStart <= fr.end;
  }
  ...
  setOptions(options) {
    ...
    // propagate dataZoom options to the DataZoom component
    if (this.dataZoom) {
      this.dataZoom.setOptions(this.options.dataZoom || {});
    }
  ...
  setItems(items) {
    ...
    if (this._isDataZoomFilterEnabled && this._isDataZoomFilterEnabled()) {
      this._setupDataZoomFilter(newDataSet);
    } else {
      this.itemSet &&
